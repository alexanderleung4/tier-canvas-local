...
- 插入式拖拽：图片可在等候区、任意层级间移动并重排；拖动时虚影会实时显示预计落点，层级本身也可拖拽重排。
...
- 自定义排行底色：可从工具栏选择任意排行内容区颜色并一键恢复默认灰色；颜色会自动保存并写入导出的 PNG。

## 更新日志

### 2026-07-17

- 新增排行底色调节器，支持本地保存、撤销、重做、刷新恢复及默认灰色重置；深浅底色会自动适配分隔线和空排行提示。
- 图片槽位和拖动预览改为透明背景，非正方形图片周围直接显示排行底色；绿色落点虚影保持不变。
- 修复拖动颜色面板时一次操作产生多条撤销记录的问题；“恢复初始设定”会保留当前排行底色，默认灰色可通过颜色旁的恢复按钮单独重置。

### 2026-07-14

- 修复 iPhone 浏览器导出的 PNG 只显示灰色图片槽位的问题；导出时改为将原图直接绘制到 Canvas，避开 iOS WebKit 可能漏载 SVG 内嵌图片的兼容性问题。

### 2026-07-13

- 优化图片拖拽反馈：源位置不再保留虚影，虚影会实时移动到预计插入位置并提前让出槽位，松手后的实际落点与预览一致。
- 新增图片聚焦预览：双击图片放大至屏幕中央，点击任意位置、按 `Esc`、`Delete` 或 `Backspace` 即可安全关闭；预览不会写入导出的 PNG。
- 修复图片拖出排行区域时可能带动同行内容错位并留下半透明残影的问题；无有效落点时会安全回到原位置，不再造成同行异常偏移。
- 新增本区作为后续功能与改动的维护记录。
