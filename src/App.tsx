...
export default function App() {
  ...
  }, [presentation, store.selectedImageId, store.undo, store.redo, store.deleteImage])
  ...
  return (
    <div className={`app-shell ${presentation ? 'presentation-mode' : ''} ${readyClass}`}>
      {!presentation && <Toolbar onClear={() => setClearOpen(true)} onExport={() => setExportOpen(true)} onPresentation={() => setPresentation(true)} />}
      ...
      <DndContext sensors={sensors} collisionDetection={collisionDetection} autoScroll onDragStart={onDragStart} onDragCancel={() => setActive(null)} onDragEnd={onDragEnd}>
        <Board presentation={presentation} activeType={active?.type ?? null} exporting={exporting} />
